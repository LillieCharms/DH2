...
const Moves = {
  ...
  aromatherapy: {
    ...
    onHit(target, source, move) {
      ...
      for (const ally of allies) {
        if ((ally === source || !(ally.hasAbility(["sapsipper", "pondweed", "lawnmowerofruin", "firedrinker"]) || ally.volatiles["substitute"] && !move.infiltrates)) && ally.cureStatus()) {
          success = true;
        }
      }
      return !!success;
  ...
  smackdown: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon) {
        let applies = !(!(pokemon.hasType("Flying") || pokemon.hasAbility(["levitate", "airbornearmor", "aircontrol", "holygrail", "risingtension", "freeflight", "hellkite", "honeymoon", "aircontrol", "magnetize", "unidentifiedflyingobject"])) || pokemon.hasItem("ironball") || pokemon.volatiles["ingrain"] || this.field.getPseudoWeather("gravity"));
