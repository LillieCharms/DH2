...
const Scripts = {
  ...
  actions: {
    ...
    hitStepTryImmunity(targets, pokemon, move) {
      ...
      for (const [i, target] of targets.entries()) {
        if (
          ...
          if (
            /*this.battle.gen >= 7 && */
            move.pranksterBoosted && !this.dex.getImmunity("prankster", target) && pokemon.hasAbility(["prankster", "openingact"]) && !targets[i].isAlly(pokemon)
          ) {
    ...
    useMoveInner(moveOrMoveName, pokemon, target, sourceEffect, zMove, maxMove) {
      ...
      if (!move.negateSecondary && !(move.hasSheerForce && pokemon.hasAbility(["sheerforce", "forceofnature", "sandwrath", "overwhelming"])) && !move.flags["futuremove"]) {
    ...
    modifyDamage(baseDamage, pokemon, target, move, suppressMessages = false) {
      ...
      if (pokemon.status === "brn" && move.category === "Physical" && !pokemon.hasAbility(["wellbakedflameorb", "feistytempo", "guts"]) && move.id !== "facade") {
  ...
  pokemon: {
    ...
    isGrounded(negateImmunity = false) {
      ...
      if (!negateImmunity && this.hasType("Flying") && !("roost" in this.volatiles))
        return false;
      if (this.hasAbility(["levitate", "holygrail", "risingtension", "freeflight", "airbornearmor", "hellkite", "honeymoon", "aircontrol", "magnetize", "unidentifiedflyingobject"]) && !this.battle.suppressingAbility(this))
        return null;
