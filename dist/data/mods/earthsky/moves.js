...
const Moves = {
  ...
  aerate: {
    ...
    onHit(target, source, move2) {
      ...
      if (veilAbilities.includes(target.getAbility()))
    ...
    type: "Flying",
    contestType: "Cute",
    onPrepareHit(target, source, move2) {
  ...
  chaoticstorm: {
    ...
    onBasePower(basePower, pokemon) {
      if (["sunnyday", "desolateland", "raindance", "primordialsea"].includes(this.field.effectiveWeather()))
  ...
  equalizer: {
    ...
    priority: 0,
    flags: { protect: 1, mirror: 1 },
    onHitField(target, source, move2) {
      const targets = [];
  ...
  escapetunnel: {
    ...
    condition: {
      ...
      onDamage(damage, target, source, effect) {
        if (effect && effect.id === "firepledge")
          return false;
      },
  ...
  preheat: {
    ...
    condition: {
      ...
      onBasePower(basePower, attacker, defender, move2) {
        if (move2.type === "Fire" || move2.twoType && move2.twoType === "Fire") {
          this.debug("preheat boost");
          return this.chainModify(2);
        }
      },
      onMoveAborted(pokemon, target, move2) {
        if ((move2.type === "Fire" || move2.twoType && move2.twoType === "Fire") && move2.id !== "preheat") {
  ...
  preservation: {
    ...
    onHit(pokemon) {
      ...
      for (const volatile of affectedStatuses) {
        ...
        if (returnVolatile) {
          if (pokemon.volatiles[volatile]) {
            ...
            if (["attract", "lockon", "mindreader", "partiallytrapped", "strongpartialtrap", "trapped"].includes(volatile)) {
            ...
            success = true;
            this.add("-start", pokemon, returnVolatile.id, "[silent]");
            pokemon.volatiles[volatile] = returnVolatile;
          }
        } else if (pokemon.volatiles[volatile]) {
          success = true;
          if (["partiallytrapped", "strongpartialtrap"].includes(volatile))
            this.add("-end", pokemon, pokemon.volatiles[volatile].sourceEffect, "[silent]");
      ...
      if (pokemon.previousTurnState.weighthg !== pokemon.weighthg) {
        ...
        if (pokemon.species.weightkg * 10 === pokemon.weighthg) {
  ...
  psybubble: {
    ...
    pp: 5,
    priority: 0,
    //This is for the actual switch. Battle.getActionSpeed refers only to this number, so it has to be the one that gets inserted
    flags: { nosleeptalk: 1, noassist: 1, failcopycat: 1, failinstruct: 1 },
    stallingMove: true,
    ...
    onHit(pokemon, move2) {
      if (pokemon.volatiles["slipaway"]) {
        ...
        if (pokemon.addVolatile("slipaway")) {
          ...
          pokemon.addVolatile("stall");
          this.queue.insertChoice({ choice: "move", pokemon, move: this.dex.moves.get("psybubble") }, true);
        }
      }
    },
    secondary: null,
    ...
    condition: {
      ...
      onTryHit(target, source, move2) {
        if (target === this.effectState.target) {
          const damage = this.actions.getDamage(source, target, move2);
          if (!damage)
            return;
  ...
  sandblast: {
    ...
    flags: { protect: 1, mirror: 1 },
    onBasePower(basePower, pokemon) {
      if (["sunnyday", "desolateland", "sandstorm"].includes(this.field.effectiveWeather()))
        return this.chainModify(1.5);
    },
    secondary: null,
    target: "normal",
    ...
    onHit(pokemon, move2) {
      if (pokemon.volatiles["slipaway"]) {
        ...
        if (pokemon.addVolatile("slipaway")) {
          pokemon.addVolatile("protect");
          pokemon.addVolatile("stall");
          this.queue.insertChoice({ choice: "move", pokemon, move: this.dex.moves.get("slipaway") }, true);
        }
      }
    },
    condition: {
  ...
  aromaticmist: {
    inherit: true,
    onModifyMove(move2, pokemon) {
      if (this.field.effectiveTerrain(pokemon) === "mistyterrain")
        move2.boosts = { spd: 2 };
    },
    target: "allies",
  ...
  beatup: {
    inherit: true,
    basePowerCallback(pokemon, target, move2) {
      return 10 + Math.floor(move2.allies.shift().species.baseStats.atk / 10);
    },
  ...
  bide: {
    ...
    condition: {
      ...
      onDamage(damage, target, source, move2) {
        if (!move2 || move2.effectType !== "Move" || !source)
          return;
  ...
  block: {
    inherit: true,
    onHit(target, source, move2) {
      return target.addVolatile("blocked", source, move2, "trapper");
    },
  ...
  bounce: {
    ...
    onTryMove(attacker, defender, move2) {
      ...
      if (attacker.removeVolatile(move2.id)) {
        return;
      }
      this.add("-prepare", attacker, move2.name);
      if (!this.runEvent("ChargeMove", attacker, defender, move2)) {
        return;
      }
    ...
    condition: {
      duration: 2,
      onInvulnerability(target, source, move2) {
        if (["twister", "thunder", "hurricane", "smackdown", "thousandarrows"].includes(move2.id)) {
          return;
        }
        return false;
      },
      onSourceBasePower(basePower, target, source, move2) {
        if (["twister", "thunder", "hurricane", "smackdown", "thousandarrows"].includes(move2.id)) {
          return this.chainModify(2);
        }
      }
  ...
  burnup: {
    ...
    self: {
      onHit(pokemon) {
        if (pokemon.hasAbility("turboblaze"))
          return;
  ...
  chloroblast: {
    ...
    onAfterMove(pokemon, target, move2) {
      if (pokemon.moveThisTurnResult != null && move2.mindBlownRecoil && !move2.multihit) {
  ...
  completeshock: {
    ...
    priority: 0,
    flags: { contact: 1, protect: 1, mirror: 1 },
    onTryMove(pokemon, target, move2) {
    ...
    self: {
      onHit(pokemon) {
        if (pokemon.hasAbility("teravolt"))
          return;
  ...
  defog: {
    ...
    onHitField(target, source, move2) {
      ...
      for (const mon of enemySide.active) {
        if (!mon.volatiles["substitute"] || move2.infiltrates)
          success = !!this.boost({ evasion: -1 }, mon);
      }
      const removeTarget = [
        "reflect",
        "lightscreen",
  ...
  dig: {
    ...
    basePower: 90,
    onTryMove(attacker, defender, move2) {
      if (!attacker.isGrounded() && !(attacker.hasType("Flying") || attacker.hasAbility("levitate")))
        return false;
      if (attacker.removeVolatile(move2.id)) {
        return;
      }
    ...
    condition: {
      duration: 2,
      onImmunity(type, pokemon) {
        if (type === "sandstorm" || type === "snow")
          return false;
      },
      onInvulnerability(target, source, move2) {
        if (["earthquake", "magnitude", "bulldoze", "dig", "fissure"].includes(move2.id)) {
          return;
        }
        return false;
      },
      onSourceModifyDamage(damage, source, target, move2) {
        if (["earthquake", "magnitude", "bulldoze", "dig"].includes(move2.id)) {
          return this.chainModify(2);
        }
      }
    },
  ...
  direclaw: {
    ...
    secondary: {
      ...
      onHit(target, source, move2) {
        ...
        if (result === 0) {
          target.trySetStatus("psn", source, move2);
        } else if (result === 1) {
          target.trySetStatus("par", source, move2);
        } else {
          target.trySetStatus("slp", source, move2);
        }
      }
    },
    contestType: "Clever",
  ...
  dive: {
    ...
    onTryMove(attacker, defender, move2) {
      ...
      if (attacker.hasAbility("gulpmissile") && attacker.species.name === "Cramorant" && !attacker.transformed) {
        ...
        attacker.formeChange(forme, move2);
      }
      this.add("-prepare", attacker, move2.name);
      if (!this.runEvent("ChargeMove", attacker, defender, move2)) {
        return;
    ...
    condition: {
      ...
      onInvulnerability(target, source, move2) {
        if (["surf", "whirlpool", "dive", "muddywater", "tidalwave"].includes(move2.id)) {
          return;
        }
        return false;
      },
  ...
  doubleteam: {
    ...
    condition: {
      ...
      onBeforeMove(pokemon) {
        pokemon.volatiles["evadestall"].duration = 2;
      },
      onAccuracy(accuracy, target, source, move2) {
        if (["allAdjacentFoes", "allAdjacent", "all"].includes(move2.target)) {
          target.removeVolatile("doubleteam");
          move2.spreadHit = true;
          return;
        }
        if (!move2.ignoreEvasion && typeof move2.accuracy === "number") {
          target.removeVolatile("doubleteam");
          return false;
        }
      },
  ...
  echoedvoice: {
    inherit: true,
    basePowerCallback(pokemon, target, move2) {
      if (!pokemon.volatiles["echoedvoice"] || move2.hit === 1) {
        pokemon.addVolatile("echoedvoice");
      }
      return this.clampIntRange(move2.basePower * pokemon.volatiles["echoedvoice"].multiplier, 1, 160);
    },
    onTry() {
    },
    condition: {
  ...
  fairylock: {
    ...
    condition: {
      ...
      onHit(target, source, move2) {
        if (move2.selfSwitch && target !== source && !source.hasItem("shedshell") && !source.hasAbility("runaway"))
          delete move2.selfSwitch;
      },
  ...
  firepledge: {
    inherit: true,
    onModifyMove(move2) {
      if (move2.sourceEffect === "waterpledge") {
        move2.type = "Water";
        move2.forceSTAB = true;
        move2.self = { sideCondition: "waterpledge" };
      }
      if (move2.sourceEffect === "grasspledge") {
        move2.type = "Grass";
        move2.forceSTAB = true;
        move2.sideCondition = "firepledge";
      }
    },
    condition: {
      duration: 4,
  ...
  flameburst: {
    ...
    onMoveFail(target, source, move2) {
      let blocked = false;
    ...
    onHit(target, source) {
      ...
      if (!target.volatiles["substitute"] || move.infiltrates) {
        ...
        if (target.volatiles["twoturnmove"] && !target.volatiles["skydrop"]) {
          if (target.removeVolatile("dig") || target.removeVolatile("dive") || target.removeVolatile("fly") || target.removeVolatile("bounce") || target.removeVolatile("phantomforce") || target.removeVolatile("shadowforce")) {
  ...
  flowershield: {
    ...
    onModifyMove(move2, pokemon) {
      if (this.field.effectiveTerrain(pokemon) === "grassyterrain")
        move2.boosts = { def: 2 };
    },
    onHitField: null,
    target: "allies",
  ...
  forestscurse: {
    ...
    onHit(target) {
      ...
      if (targetTypes.length > 1 && targetTypes[1] === "Grass" || targetTypes.join() === "Grass")
        return false;
      if (targetTypes[0] === "Grass") {
        if (target.setType("Grass"))
          succeeded = true;
      } else {
        if (target.setType([targetTypes[0], "Grass"]), true)
          succeeded = true;
      }
      if (succeeded)
        this.add("-start", target, "typechange", target.getTypes().join("/"), "[from] move: Forest's Curse");
      else {
  ...
  grasspledge: {
    ...
    onModifyMove(move2) {
      ...
      if (move2.sourceEffect === "firepledge") {
        move2.type = "Fire";
        move2.forceSTAB = true;
        move2.sideCondition = "firepledge";
      }
  ...
  grassyterrain: {
    ...
    condition: {
      ...
      onBasePower(basePower, attacker, defender, move2) {
        ...
        if (weakenedMoves.includes(move2.id)) {
          this.debug("move weakened by grassy terrain");
          return this.chainModify(0.5);
        }
        if ((move2.type === "Grass" || move2.twoType && move2.twoType === "Grass") && attacker.isGrounded()) {
      ...
      onAfterMoveSecondary(target, source, move2) {
        if (["firespin", "firepledge", "inferno", "searingshot", "napalm", "burnup", "overheat", "blastburn"].includes(move2.id)) {
          target.side.addSideCondition("firepledge");
          this.field.clearTerrain();
        }
        if (["whirlpool", "waterpledge", "muddywater", "surf", "originpulse", "tidalwave", "hydrocannon", "waterspout"].includes(move2.id)) {
  ...
  healblock: {
    ...
    condition: {
      ...
      onBeforeMove(pokemon, target, move2) {
        if (move2.flags["heal"]) {
          this.add("cant", pokemon, "move: Heal Block", move2);
          return false;
        }
      },
  ...
  iceball: {
    ...
    basePower: 40,
    basePowerCallback(pokemon, target, move2) {
      if (!pokemon.volatiles["iceball"] || move2.hit === 1) {
        pokemon.addVolatile("iceball");
      }
      return this.clampIntRange(move2.basePower * pokemon.volatiles["iceball"].multiplier, 1, 160);
    },
    category: "Physical",
    name: "Ice Ball",
    pp: 20,
  ...
  ingrain: {
    inherit: true,
    onTryMove(pokemon) {
      if (!pokemon.isGrounded() && !(pokemon.hasType("Flying") || pokemon.hasAbility("levitate")))
        return false;
    },
    condition: {
      onStart(pokemon) {
  ...
  knockoff: {
    inherit: true,
    onBasePower(basePower, source, target, move2) {
    },
    //Clears x1.5 boost
  ...
  lastrespects: {
    ...
    basePower: 0,
    basePowerCallback(pokemon, target, move2) {
      return 10 + Math.floor(move2.allies.shift().species.baseStats.spa / 10);
    },
    category: "Special",
    onModifyMove(move2, pokemon) {
      move2.allies = pokemon.side.pokemon.filter((ally) => ally === pokemon || ally.fainted);
  ...
  lockon: {
    ...
    onTryHit(target, source) {
      if (source.volatiles["lockon"]) {
        if (source.volatiles["lockon"].source === target)
          return false;
    ...
    condition: {
      ...
      onInvulnerability(target, source, move2) {
        if (move2 && source === this.effectState.target && target === this.effectState.source)
          return 0;
      },
      onModifyMove(move2, source, target) {
        if (move2 && source === this.effectState.target && target === this.effectState.source) {
          move2.accuracy = true;
          move2.ignoreEvasion = true;
        }
  ...
  magicpowder: {
    ...
    onHit(target) {
      let succeeded = false;
      const targetTypes = target.getTypes();
      if (targetTypes.length > 1 && targetTypes[1] === "Psychic" || targetTypes.join() === "Psychic")
        return false;
      if (targetTypes[0] === "Psychic") {
        if (target.setType("Psychic"))
          succeeded = true;
      } else {
        if (target.setType([targetTypes[0], "Psychic"]), true)
          succeeded = true;
      }
      if (succeeded)
        this.add("-start", target, "typechange", target.getTypes().join("/"), "[from] move: Magic Powder");
      else {
        this.add("-fail", target);
        return null;
      }
  ...
  magneticflux: {
    ...
    onHitSide(side, source, move2) {
      ...
      for (const target of targets) {
        didSomething = this.boost({ atk: 1, def: 1, spa: 1, spd: 1 }, target, source, move2, false, true) || didSomething;
      }
      return didSomething;
    },
  ...
  meanlook: {
    ...
    onHit(target, source, move2) {
      return target.addVolatile("meanlooked", source, move2, "trapper");
    },
  ...
  mindreader: {
    ...
    onTryHit(pokemon) {
      if (pokemon.volatiles["mindreader"])
        return false;
    },
    condition: {
      onStart(pokemon) {
  ...
  minimize: {
    ...
    condition: {
      ...
      onSourceModifyDamage(damage, source, target, move2) {
        ...
        if (boostedMoves.includes(move2.id)) {
          return this.chainModify(2);
        }
      },
      onAccuracy(accuracy, target, source, move2) {
    ...
    condition: {
      ...
      onModifyMove(move2) {
        ...
        if (!move2.ignoreImmunity)
          move2.ignoreImmunity = {};
        if (move2.ignoreImmunity !== true) {
          move2.ignoreImmunity["Psychic"] = true;
        }
        move2.pranksterBoosted = false;
      },
      onEnd(pokemon) {
    ...
    condition: {
      ...
      onTryHit(target, source, move2) {
        if (!move2.flags["protect"] || move2.category === "Status") {
          if (move2.isZ || move2.isMax && !move2.breaksProtect)
  ...
  orderup: {
    ...
    basePower: 60,
    onModifyMove(move2, pokemon) {
      if (pokemon.volatiles["commanded"] && pokemon.hasAbility("sheerforce"))
        move2.hasSheerForce = true;
    },
    onAfterMoveSecondarySelf(pokemon, target, move2) {
      if (!pokemon.volatiles["commanded"] || move2.hasSheerForce)
        return;
  ...
  phantomforce: {
    ...
    condition: {
      duration: 2,
      onInvulnerability(target, source, move2) {
        if (["phantomforce", "shadowclaw", "shadowforce", "shadowpunch", "shadowsneak"].includes(move2.id)) {
          return;
        }
        return false;
      },
  ...
  poltergeist: {
    ...
    onPrepareHit(target, source, move2) {
      ...
      if (!(item && this.singleEvent("TakeItem", item, target.itemData, target, source, move2, item)))
        return false;
      if (!item.fling)
        return false;
      move2.basePower = 80 + item.fling.basePower;
      target.item = item.id;
    },
  ...
  rage: {
    ...
    condition: {
      ...
      onHit(target, source, move2) {
        if (target !== source && move2.category !== "Status") {
  ...
  revivalblessing: {
    ...
    onDisableMove(pokemon) {
      if (!pokemon.side.pokemon.filter((ally) => ally.fainted).length)
        pokemon.disableMove("revivalblessing");
    }
  },
  roaroftime: {
    inherit: true,
  ...
  roost: {
    ...
    pp: 10,
    onTryMove(pokemon) {
      if (!pokemon.isGrounded() && (pokemon.volatiles["magnetrise"] || pokemon.volatiles["risingchorus"] || pokemon.volatiles["telekinesis"] || !pokemon.ignoringItem() && pokemon.getItem() === "airballoon"))
        return false;
    },
  ...
  skydrop: {
    inherit: true,
    onPrepareHit(target, source, move2) {
      if (!source.canFloat() || !target.canFloat())
        return false;
    },
    onTryHit(target, source, move2) {
      if (target.fainted)
        return false;
      if (source.removeVolatile(move2.id)) {
        ...
        this.add("-prepare", source, move2.name, target);
    ...
    condition: {
      ...
      onFoeBeforeMove(attacker, defender, move2) {
      ...
      onAnyInvulnerability(target, source, move2) {
      ...
      onAnyBasePower(basePower, target, source, move2) {
        if (target !== this.effectState.target && target !== this.effectState.source) {
          return;
        }
        if (source === this.effectState.target && target === this.effectState.source) {
  ...
  snatch: {
    ...
    condition: {
      ...
      onAnyTryMove(source, target, move2) {
        const snatchUser = this.effectState.source;
        if (snatchUser.isSkyDropped())
          return;
        if (!move2 || !move2.flags["snatch"] || move2.sourceEffect === "snatch") {
          return;
        }
        ...
        this.actions.useMove(move2.id, snatchUser);
        return null;
      }
    },
    isNonstandard: null
  ...
  soak: {
    ...
    onHit(target) {
      const targetTypes = target.getTypes();
      let succeeded = false;
      if (targetTypes.length > 1 && targetTypes[1] === "Water" || targetTypes.join() === "Water")
        return false;
      if (targetTypes[0] === "Water") {
        if (target.setType("Water"))
          succeeded = true;
      } else {
        if (target.setType([targetTypes[0], "Water"]), true)
          succeeded = true;
      }
      if (succeeded)
        this.add("-start", target, "typechange", target.getTypes().join("/"), "[from] move: Soak");
      else {
        this.add("-fail", target);
        return null;
      }
  ...
  spikes: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (!pokemon.isGrounded())
          return;
        const damageAmounts = [0, 15, 20, 24];
  ...
  teatime: {
    ...
    onHitField(target, source, move2) {
      ...
      for (const active of this.getAllActive()) {
        if (this.runEvent("Invulnerability", active, source, move2) === false) {
  ...
  terablast: {
    ...
    onPrepareHit(target, source, move2) {
      this.attrLastMove("[anim] Tera Blast " + move2.type);
    },
    onModifyType(move2, pokemon, target) {
      if (pokemon.ignoringItem())
        return;
      ...
      if (itemname.endsWith("Tera Shard")) {
        if (itemname === "Stellar Tera Shard") {
          if (pokemon.species.baseSpecies === "Terapagos" && pokemon.species.forme === "Stellar") {
            ...
            move2.type = this.getBestEffectiveness(pokemon, target);
          }
        } else {
          move2.type = itemname.substr(0, itemname.length - 11);
        }
      }
    },
    onModifyMove(move2, pokemon) {
  ...
  toxicthread: {
    ...
    flags: { protect: 1, reflectable: 1, mirror: 1 },
    onHit(target, source, move2) {
      let success = target.trySetStatus("psn", source, move2);
      if (target.addVolatile("trapped", source, move2, "trapper"))
        success = true;
      return success;
    },
    secondary: null,
  ...
  trickortreat: {
    ...
    onHit(target) {
      let succeeded = false;
      const targetTypes = target.getTypes();
      if (targetTypes.length > 1 && targetTypes[1] === "Ghost" || targetTypes.join() === "Ghost")
        return false;
      if (targetTypes[0] === "Ghost") {
        if (target.setType("Ghost"))
          succeeded = true;
      } else {
        if (target.setType([targetTypes[0], "Ghost"]), true)
          succeeded = true;
      }
      if (succeeded)
        this.add("-start", target, "typechange", target.getTypes().join("/"), "[from] move: Trick-or-Treat");
      else {
        this.add("-fail", target);
        return null;
      }
  ...
  attract: {
    ...
    condition: {
      ...
      onBeforeMove(pokemon, target, move2) {
        if (pokemon.volatiles["nointerrupt"]?.ignore.includes("attract"))
          return;
  ...
  charge: {
    ...
    condition: {
      onBasePowerPriority: 9,
      onBasePower(basePower, attacker, defender, move2) {
        if (move2.type === "Electric" || move2.twoType && move2.twoType === "Electric") {
          this.debug("charge boost");
          return this.chainModify(2);
        }
      },
      onMoveAborted(pokemon, target, move2) {
        if ((move2.type === "Electric" || move2.twoType && move2.twoType === "Electric") && move2.id !== "charge") {
  ...
  disable: {
    ...
    condition: {
      ...
      onBeforeMove(attacker, defender, move2) {
        if (move2.id === this.effectState.move && !(attacker.volatiles["nointerrupt"] && attacker.volatiles["nointerrupt"].ignore.includes("healblock"))) {
          this.add("cant", attacker, "Disable", move2);
          return false;
        }
      },
      onDisableMove(pokemon) {
  ...
  electricterrain: {
    ...
    condition: {
      ...
      onBasePowerPriority: 6,
      onBasePower(basePower, attacker, defender, move2) {
        if ((move2.type === "Electric" || move2.twoType && move2.twoType === "Electric") && attacker.isGrounded() && !attacker.isSemiInvulnerable()) {
  ...
  electrify: {
    ...
    condition: {
      ...
      onModifyType(move2) {
        if (move2.id !== "struggle") {
          this.debug("Electrify making move type electric");
          move2.type = "Electric";
          if (move2.twoType)
            delete move2.twoType;
        }
      }
    },
    isNonstandard: null
  ...
  encore: {
    ...
    condition: {
      ...
      onStart(target) {
        ...
        if (!move2)
          return false;
        const moveIndex = target.moves.indexOf(move2.id);
        if (move2.flags["failencore"] || !target.moveSlots[moveIndex] || target.moveSlots[moveIndex].pp <= 0) {
          return false;
        }
        this.effectState.move = move2.id;
        this.add("-start", target, "Encore");
        if (!this.queue.willMove(target)) {
          this.effectState.duration++;
        }
      },
      onOverrideAction(pokemon, target, move2) {
        if (!pokemon.volatiles["nointerrupt"]?.ignore.includes("encore") && move2.id !== this.effectState.move)
  ...
  fling: {
    ...
    onTryMove(source, target, move2) {
      ...
      if (item.fling && item.fling.flags) {
        Object.keys(item.fling.flags).forEach((key) => {
          move2.flags[key] = item.fling.flags[key];
        });
      }
    },
    onPrepareHit(target, source, move2) {
      if (source.ignoringItem())
        return false;
      const item = source.getItem();
      if (!this.singleEvent("TakeItem", item, source.itemData, source, source, move2, item))
        return false;
      if (!item.fling)
        return false;
      ...
      if (item.isBerry) {
        ...
        if (item.fling.boosts) {
          move2.secondaries.push({ boosts: item.fling.boosts });
          move2.secondaries.push({ chance: 100 });
        }
      }
  ...
  geomancy: {
    ...
    onTryMove(attacker, defender, move2) {
      if (attacker.removeVolatile(move2.id) || move2.sourceEffect === "snatch") {
        return;
      }
      this.add("-prepare", attacker, move2.name);
      if (!this.runEvent("ChargeMove", attacker, defender, move2)) {
  ...
  hiddenpowerfairy: {
    //Used only in Glyphic Spell Adapt... for now.
    num: 237,
    accuracy: 100,
    basePower: 60,
    category: "Special",
  ...
  imprison: {
    ...
    condition: {
      ...
      onFoeBeforeMove(attacker, defender, move2) {
        if (move2.id !== "struggle" && this.effectState.source.hasMove(move2.id) && (attacker.volatiles["nointerrupt"] && !attacker.volatiles["nointerrupt"].imprisoned[this.effectState.source])) {
          this.add("cant", attacker, "move: Imprison", move2);
          return false;
        }
      }
    }
  ...
  mirrormove: {
    ...
    onTryHit(target, source) {
      ...
      if (!move2.flags["mirror"]) {
        return false;
      }
      this.actions.useMove(move2.id, source, target);
      return null;
    },
    isNonstandard: null,
  ...
  mistyterrain: {
    ...
    condition: {
      ...
      onBasePowerPriority: 6,
      onBasePower(basePower, attacker, defender, move2) {
        if ((move2.type === "Dragon" || move2.twoType && move2.twoType === "Dragon") && defender.isGrounded() && !defender.isSemiInvulnerable()) {
  ...
  perishsong: {
    ...
    onHitField(target, source, move2) {
      ...
      let message = false;
      for (const pokemon of this.getAllActive()) {
        if (this.runEvent("Invulnerability", pokemon, source, move2) === false) {
          this.add("-miss", source, pokemon);
          result = true;
        } else if (this.runEvent("TryHit", pokemon, source, move2) === null) {
          result = true;
  ...
  psychicterrain: {
    ...
    condition: {
      ...
      onBasePowerPriority: 6,
      onBasePower(basePower, attacker, defender, move2) {
        if ((move2.type === "Psychic" || move2.twoType && move2.twoType === "Psychic") && attacker.isGrounded() && !attacker.isSemiInvulnerable()) {
  ...
  sketch: {
    ...
    onHit(target, source) {
      const move2 = target.lastMove;
      if (source.transformed || !move2 || source.moves.includes(move2.id))
        return false;
      if (move2.noSketch)
        return false;
      ...
      this.add("-activate", source, "move: Sketch", move2.name);
    },
  ...
  taunt: {
    ...
    condition: {
      ...
      onBeforeMove(attacker, defender, move2) {
        if (move2.category === "Status" && move2.id !== "mefirst" && !attacker.volatiles["nointerrupt"]?.ignore.includes("taunt")) {
  ...
  wideguard: {
    ...
    condition: {
      ...
      onTryHitPriority: 4,
      onTryHit(target, source, move2) {
        if (move2?.target !== "allAdjacent" && move2.target !== "allAdjacentFoes" && move2.name !== "Burst") {
          return;
        }
  ...
  assist: {
    ...
    onHit(target) {
      ...
      for (const pokemon of target.side.pokemon) {
        if (pokemon === target || pokemon.hasAbility("owntempo"))
          continue;
        for (const moveSlot of pokemon.moveSlots) {
          const moveid = moveSlot.id;
          if (move2.flags["noassist"])
            continue;
          const move2 = this.dex.moves.get(moveid);
          moves.push(moveid);
        }
      }
      let randomMove = "";
  ...
  copycat: {
    ...
    onHit(pokemon) {
      ...
      if (move2.flags["failcopycat"]) {
        return false;
      }
      this.actions.useMove(move2.id, pokemon);
    },
  ...
  mefirst: {
    ...
    onTryHit(target, source) {
      ...
      const move2 = this.dex.getActiveMove(action.move.id);
      if (move2.uncopyable) {
        this.add("-activate", target, "ability: Own Tempo");
        this.hint("Own Tempo blocks effects that steal or copy its moves");
        return null;
      }
      if (move2.category === "Status" || move2.flags["failmefirst"])
        return false;
      source.addVolatile("mefirst");
      this.actions.useMove(move2, source, target);
      return null;
    },
    isNonstandard: null,
  ...
  mimic: {
    ...
    onHit(target, source) {
      ...
      if (move2.flags["failmimic"] || source.moves.includes(move2.id)) {
        return false;
      }
      ...
      this.add("-start", source, "Mimic", move2.name);
    },
    ...
    onHit(pokemon) {
      ...
      for (const moveSlot of pokemon.moveSlots) {
        ...
        if (!moveid)
          continue;
        const move2 = this.dex.moves.get(moveid);
        if (move2.flags["nosleeptalk"] || move2.flags["charge"]) {
          continue;
        }
        moves.push(moveid);
      }
