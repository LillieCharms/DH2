...
var import_pokemon2 = require("../../../sim/pokemon");
...
const Scripts = {
  teambuilderConfig: {
    ...
    moveIsNotUseless(id, species, moves, set, dex) {
      ...
      const formatType = dex.modid.indexOf("triples") > 0 ? "triples" : "singles";
      if (itemid === "pidgeotite")
        abilityid = "noguard";
      if (itemid === "blastoisinite" || itemid === "magmortarite")
        abilityid = "megalauncher";
      if (itemid === "heracronite")
        abilityid = "skilllink";
      if (itemid === "cameruptite")
        abilityid = "sheerforce";
      if (itemid === "aerodactylite" || itemid === "charizarditex")
        abilityid = "toughclaws";
      if (itemid === "ralitite" || itemid === "charizarditey")
        abilityid = "drought";
      if (itemid === "glalitite")
        abilityid = "refrigerate";
      if (itemid === "galladite")
        abilityid = "sharpness";
      if (itemid === "sharpedonite")
        abilityid = "strongjaw";
      if (species.baseSpecies === "Unown") {
        if (id.startsWith("hiddenpower"))
          return id === (species.forme ? "hiddenpowerpsychic" : "hiddenpower");
        else
          return ["G", "P", "Q"].includes(species.forme);
      }
      switch (id) {
        case "chargebeam":
        case "fierydance":
        ...
        case "bellydrum":
          return moves.includes("aquajet") || moves.includes("extremespeed") || ["iceface", "unburden"].includes(abilityid);
        case "bodypress":
        ...
        case "hypnosis":
          return ["baddreams", "compoundeyes", "insomnia"].includes(abilityid) && !(moves.includes("spore") || moves.includes("darkvoid") || moves.includes("grasswhistle") || moves.includes("sing") || moves.includes("sleeppowder"));
        case "ironhead":
  ...
  /* sim edits */
  pokemon: {
    previousTurnState: import_pokemon.EffectState,
    turnState: import_pokemon.EffectState,
    updateTurnState() {
    ...
    getSmartTargets(target, move) {
      ...
      if (target.adjacentAllies().length === 2 && (this.tacticianBoosted || this.position === 1)) {
        const targetPos = this.battle.sample([0, 1]);
        ...
        if (target === this || !target.hp || !target.isValidTarget()) {
          target = target.adjacentAllies()[1 - targetPos];
          if (!target || target === this || !target.hp || !target.isValidTarget()) {
            move.smartTarget = false;
            return [target2];
          }
        }
    ...
    getMoveTargets(move, target) {
      ...
      switch (move.target) {
        ...
        default:
          ...
          if (target.side.active.length > 1 && !move.tracksTarget) {
            const isCharging = move.flags["charge"] && !this.volatiles["twoturnmove"] && !(move.id.startsWith("solarb") && this.battle.field.isWeather(["sunnyday", "desolateland"])) && !((this.hasItem("powerherb") || !this.hasAbility("tireless")) && move.id !== "skydrop");
            if (!isCharging) {
      ...
      targets.filter((target2) => !target2.volatiles["commanding"]);
    ...
    getMoves(lockedMove, restrictData) {
      ...
      for (const moveSlot of this.moveSlots) {
        ...
        if (moveSlot.pp <= 0 && !this.volatiles["partialtrappinglock"] || disabled && this.side.active.length >= 2 && this.battle.actions.targetTypeChoices(target)) {
          disabled = true;
        }
        if (!disabled) {
          hasValidMove = true;
    ...
    getStrongestMove(target, seeItem) {
      let strongestMove = void 0;
      for (const moveSlot of this.moveSlots) {
        let bestBP = 1;
        ...
        let direct = false;
        if (["powertrip", "punishment", "storedpower", "trumpcard"].includes(move.id) || !bp && move.category !== "Status") {
          switch (move.id) {
            case "dragonrage":
            case "sonicboom":
            ...
            case "fling":
              if (seeItem) {
                const item2 = this.getItem();
                bp = item2.fling ? item2.fling.basePower : 0;
              } else {
                bp = 20;
              }
              break;
            case "naturalgift":
              if (seeItem) {
                const item2 = this.getItem();
                bp = item2.naturalGift ? item2.naturalGift.basePower : 0;
              } else {
                bp = 70;
              }
              break;
        ...
        if (!direct) {
          if (move.onModifyType)
            this.battle.singleEvent("ModifyType", move, null, this, target);
    ...
    isGrounded(negateImmunity = false, smackDownCheck = false) {
      if (!this.canFloat())
        return true;
      if (this.volatiles["roost"] || this.volatiles["dig"] || this.volatiles["dive"])
        return true;
      if (this.volatiles["magnetrise"] || this.volatiles["risingchorus"] || this.volatiles["telekinesis"])
        return false;
      if (!smackDownCheck && this.volatiles["smackdown"])
        return true;
      if (!negateImmunity && this.hasType("Flying"))
        return false;
    ...
    canFloat() {
      ...
      if (this.hasAbility("suctioncups") || this.hasAbility("heavymetal"))
        return false;
      if ("gravity" in this.battle.field.pseudoWeather)
        return false;
      if (this.volatiles["ingrain"])
        return false;
      const item = this.ignoringItem() ? "" : this.item;
      if (item === "ironball")
        return false;
    ...
    getMoveRequestData() {
      ...
      if (!lockedMove) {
        if (this.canMegaEvo && !magicRoom)
          data.canMegaEvo = true;
        if (this.canUltraBurst && !magicRoom)
    ...
    cureStatus(silent = false) {
      if (!this.hp || !this.status)
        return false;
      const result = this.battle.runEvent("RemoveStatus", this, null, null, this.status);
      if (!result) {
    ...
    trySetStatus(status2, source = null, sourceEffect = null) {
      ...
      if (source && source !== this && !source.ignoringAbility()) {
        ...
        if (source.hasAbility("corrosion") && ["tox", "psn"].includes(status2.id || status2))
          ignoreImmunities = true;
      }
      return this.setStatus(this.status || status2, source, sourceEffect, ignoreImmunities);
    },
    setStatus(status2, source = null, sourceEffect = null, ignoreImmunities = false) {
      if (!this.hp)
        return false;
      status2 = this.battle.dex.conditions.get(status2);
      ...
      if (!ignoreImmunities && status2.id) {
        if (!this.runStatusImmunity(status2.id === "tox" ? "psn" : status2.id)) {
      ...
      if (status2.id) {
        const result = this.battle.runEvent("SetStatus", this, source, sourceEffect, status2);
        if (!result) {
          this.battle.debug("set status [" + status2.id + "] interrupted");
          return result;
        }
      }
      this.status = status2.id;
      this.statusState = { id: status2.id, target: this };
      if (source)
        this.statusState.source = source;
      if (status2.duration)
        this.statusState.duration = status2.duration;
      if (status2.durationCallback) {
        this.statusState.duration = status2.durationCallback.call(this.battle, this, source, sourceEffect);
      }
      if (status2.id && !this.battle.singleEvent("Start", status2, this.statusState, this, source, sourceEffect)) {
        this.battle.debug("status start [" + status2.id + "] interrupted");
        this.status = prevStatus;
        this.statusState = prevStatusState;
        return false;
      }
      if (status2.id && !this.battle.runEvent("AfterSetStatus", this, source, sourceEffect, status2)) {
        return false;
      }
      return true;
    },
    addVolatile(status2, source = null, sourceEffect = null, linkedStatus = null) {
      let result;
      status2 = this.battle.dex.conditions.get(status2);
      if (!this.hp && !status2.affectsFainted)
        return false;
      ...
      if (!source)
        source = this;
      if (this.volatiles[status2.id]) {
        if (!status2.onRestart)
          return false;
        return this.battle.singleEvent("Restart", status2, this.volatiles[status2.id], this, source, sourceEffect);
      }
      if (!(source && !source.ignoringAbility() && source.hasAbility("myceliummight")) && !this.runStatusImmunity(status2.id)) {
      ...
      result = this.battle.runEvent("TryAddVolatile", this, source, sourceEffect, status2);
      if (!result) {
        this.battle.debug("add volatile [" + status2.id + "] interrupted");
        return result;
      }
      this.volatiles[status2.id] = { id: status2.id };
      this.volatiles[status2.id].target = this;
      if (source) {
        this.volatiles[status2.id].source = source;
        this.volatiles[status2.id].sourcePosition = source.position;
      }
      if (sourceEffect)
        this.volatiles[status2.id].sourceEffect = sourceEffect;
      if (status2.duration)
        this.volatiles[status2.id].duration = status2.duration;
      if (status2.durationCallback) {
        this.volatiles[status2.id].duration = status2.durationCallback.call(this.battle, this, source, sourceEffect);
      }
      result = this.battle.singleEvent("Start", status2, this.volatiles[status2.id], this, source, sourceEffect);
      if (!result) {
        delete this.volatiles[status2.id];
        return result;
      }
      if (linkedStatus && source) {
        if (!source.volatiles[linkedStatus.toString()]) {
          ...
          source.volatiles[linkedStatus.toString()].linkedStatus = status2;
        } else {
          source.volatiles[linkedStatus.toString()].linkedPokemon.push(this);
        }
        this.volatiles[status2.toString()].linkedPokemon = [source];
        this.volatiles[status2.toString()].linkedStatus = linkedStatus;
      }
      return true;
    },
    removeVolatile(status2) {
      if (!this.hp)
        return false;
      status2 = this.battle.dex.conditions.get(status2);
      if (!this.volatiles[status2.id])
        return false;
      if (this.volatiles["stasis"]?.affectedStatuses.includes(status2.id)) {
        this.battle.debug("remove volatile [" + status2.id + "] interrupted");
        return false;
      }
      const result = this.battle.singleEvent("TryRemoveVolatile", this, null, null, this.status);
      if (!result) {
        this.battle.debug("remove volatile [" + status2.id + "] interrupted");
        return false;
      }
      this.battle.singleEvent("End", status2, this.volatiles[status2.id], this);
      const linkedPokemon = this.volatiles[status2.id].linkedPokemon;
      const linkedStatus = this.volatiles[status2.id].linkedStatus;
      delete this.volatiles[status2.id];
      if (linkedPokemon) {
    ...
    ignoringAbility() {
      ...
      for (const pokemon of this.battle.getAllActive()) {
        if ((pokemon.ability === "neutralizinggas" || pokemon.ability === "glyphicspell" && pokemon.abilityState.unownType === "N") && !pokemon.volatiles["gastroacid"] && !pokemon.abilityState.ending) {
          neutralizinggas = true;
          break;
        }
      }
      return !!(!this.isActive || (this.volatiles["gastroacid"] || neutralizinggas && this.ability !== "neutralizinggas" && !(this.ability === "glyphicspell" && this.abilityState.unownType === "N")) && !this.getAbility().isPermanent);
    },
    ...
    transformInto(pokemon, effect) {
      ...
      if (pokemon.species.baseSpecies === "Unown" && this.species.baseSpecies === "Unown" && pokemon.abilityState.unownType) {
        ...
        delete this.abilityState.formeDecided;
      }
      if (effect) {
  ...
  field: {
    suppressingWeather() {
      if ("midnight" in this.pseudoWeather)
        return true;
    ...
    setTerrain(status2, source = null, sourceEffect = null) {
      status2 = this.battle.dex.conditions.get(status2);
      ...
      const result = this.battle.runEvent("SetTerrain", source, source, status2);
      if (!result) {
        if (result === false) {
      ...
      if (this.terrain === status2.id)
        return false;
      const prevTerrain = this.terrain;
      const prevTerrainState = this.terrainState;
      this.terrain = status2.id;
      this.terrainState = {
        id: status2.id,
        source,
        sourceSlot: source.getSlot(),
        duration: status2.duration
      };
      if (status2.durationCallback) {
        this.terrainState.duration = status2.durationCallback.call(this.battle, source, source, sourceEffect);
      }
      if (!this.battle.singleEvent("FieldStart", status2, this.terrainState, this, source, sourceEffect)) {
    ...
    chooseSwitch(slotText) {
      ...
      if (this.requestState === "move") {
        if (pokemon.trapped) {
          ...
          const status2 = this.emitChoiceError(`Can't switch: The active Pok\xE9mon is trapped`, includeRequest);
          if (includeRequest)
  ...
  getTarget(pokemon, move, targetLoc, originalTarget) {
    ...
    if (["adjacentAlly", "any", "normal"].includes(move.target) && targetLoc === selfLoc && !pokemon.volatiles["twoturnmove"] && !pokemon.volatiles["rollout"]) {
  ...
  getRandomTarget(pokemon, move) {
    ...
    if (pokemon.side.foe.active.length === 1 && pokemon.side.foe.active[0].volatiles["playdead"])
      return null;
  ...
  singleEvent(eventid, effect, state, target, source, sourceEffect, relayVar, customCallback) {
    ...
    if (effect.effectType === "Status" && target instanceof import_pokemon2.Pokemon && target.status !== effect.id) {
      return relayVar;
    }
    if (eventid !== "Start" && eventid !== "TakeItem" && eventid !== "Primal" && effect.effectType === "Item" && target instanceof import_pokemon2.Pokemon && target.ignoringItem()) {
      this.debug(eventid + " handler suppressed by Klutz or Magic Room");
      return relayVar;
    }
    if (eventid !== "End" && effect.effectType === "Ability" && target instanceof import_pokemon2.Pokemon && target.ignoringAbility()) {
    ...
    if ((effect.effectType === "Terrain" || eventid === "Terrain") && eventid !== "FieldStart" && eventid !== "FieldResidual" && eventid !== "FieldEnd" && this.field.suppressingTerrain()) {
  ...
  runEvent(eventid, target, source, sourceEffect, relayVar, onEffect, fastExit) {
    ...
    if (source instanceof import_pokemon2.Pokemon)
      effectSource = source;
    ...
    if (["Invulnerability", "TryHit", "DamagingHit", "EntryHazard"].includes(eventid)) {
      handlers.sort(import_battle.Battle.compareLeftToRightOrder);
    } else if (fastExit) {
      handlers.sort(import_battle.Battle.compareRedirectOrder);
    } else {
    ...
    for (const handler of handlers) {
      ...
      if (eventid !== "Start" && eventid !== "SwitchIn" && eventid !== "TakeItem" && effect.effectType === "Item" && effectHolder instanceof import_pokemon2.Pokemon && effectHolder.ignoringItem()) {
        if (eventid !== "Update") {
          this.debug(eventid + " handler suppressed by Klutz or Magic Room");
        }
        continue;
      } else if (eventid !== "End" && effect.effectType === "Ability" && effectHolder instanceof import_pokemon2.Pokemon && effectHolder.ignoringAbility()) {
        if (eventid !== "Update") {
          this.debug(eventid + " handler suppressed by Gastro Acid or Neutralizing Gas");
        }
        continue;
      ...
      if ((effect.effectType === "Terrain" || eventid === "Terrain") && eventid !== "Residual" && eventid !== "End" && this.field.suppressingTerrain()) {
        this.debug(eventid + " handler suppressed by Climate Break or Midnight");
        continue;
      }
      let returnVal;
  ...
  actions: {
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, sourceEffect, zMove, externalMove, maxMove, originalTarget) {
      ...
      if (!externalMove) {
        ...
        if (lockedMove === true)
          lockedMove = false;
        if (!lockedMove) {
          if (!pokemon.deductPP(baseMove, null, target) && move.id !== "struggle" && !pokemon.volatiles["nointerrupt"]) {
    ...
    canMegaEvo(pokemon) {
      if ("magicroom" in this.battle.field.pseudoWeather || pokemon.happiness < 180)
        return null;
    ...
    canUltraBurst(pokemon) {
      if ("magicroom" in this.battle.field.pseudoWeather)
        return null;
  ...
  getImmunity(source, target) {
    ...
    if (["earthsky", "gen9eshorizons"].includes(this.currentMod)) {
      ...
      sourceType = typeof source !== "string" ? source.type : source;
  ...
  getBestEffectiveness(source, target, resistFirst) {
    const dex = this.dex;
    ...
    if (resistFirst) {
      if (Array.isArray(targetTyping)) {
        ...
        typesTest = bestTypes.filter((type) => !dex.getImmunity(targetTyping, type));
      ...
      if (typesTest.length) {
        ...
        if (Array.isArray(targetTyping)) {
          ...
          for (const targetType in targetTyping) {
            typesTest = typesTest.concat(bestTypes.filter((type) => dex.getEffectiveness(targetType, type) < 0));
          }
          bestTypes = Object.assign([], typesTest);
        } else {
        ...
        if (bestTypes.length) {
          ...
          if (targetTyping.length) {
            typesTest = [];
            for (const targetType in targetTyping) {
              typesTest = typesTest.concat(bestTypes.filter((type) => dex.getEffectiveness(targetType, type) === -2));
            }
          } else
            typesTest = bestTypes.filter((type) => dex.getEffectiveness(targetTyping, type) === -2);
          ...
          bestTypes = Object.assign([], typeList);
      }
      ...
      if (typesTest.length) {
        ...
        if (Array.isArray(targetTyping)) {
          ...
          typesTest = bestTypes.filter((type) => dex.getEffectiveness(targetTyping, type) < 0);
  ...
  init() {
    const baseSeven = [
      //Pokemon modified to use a relative's Gen VII learnset as a base
      "meowthgalar",
      "perrserker",
      "growlithehisui",
      "arcaninehisui",
    ...
    const baseEight = [
      //Pokemon using their Gen VIII learnsets as a base
      "bulbasaur",
      "ivysaur",
      "venusaur",
      "charmander",
    ...
    const baseNine = [
      //Pokemon using their Gen IX learnsets as a base
      "wooper",
      "wooperpaldea",
      "quagsire",
      "riolu",
    ...
    const renamedMoves = [
      //Also includes the replacement of Axe Kick and Hail with Jump Kick and Snowscape, respectively
      "axekick",
      "banefulbunker",
      "chillingwater",
      "clangoroussoul",
    ...
    const droppedMachines = [
      //Machines dropped from Earth & Sky; includes Flash, Natural Gift, and Toxic, which are re-added to their Pokemon after the algorithm, and the few moves which are still Tutored since they're handled separately
      "agility",
      "aircutter",
      "airslash",
      "alluringvoice",
    ...
    const noUniversalTMs = [
      //Only Gen 5+ since it's the earliest to lose a universal TM
      "smeargle",
      "tynamo",
      "scatterbug",
      "spewpa",
    ...
    for (let pokemon of this.species.all()) {
      ...
      const learnsetTest = false;
      const formatsTest = false;
      if (formatsTest)
        console.log(pokemonID);
      if (pokemon === null || pokemon.num < -500 || pokemon.forme && (["Egelas", "Sartori", "Hassrim", "Mega-Statue"].includes(pokemon.forme) || ["tynamoschool", "indeedeehassrimf"].includes(pokemonID) || pokemon.forme === "Mega" && ["Butterfree", "Slowking", "Torkoal", "Milotic", "Electivire", "Magmortar", "Garbodor", "Beheeyem", "Sandaconda", "Alcremie", "Froslass", "Druddigon"].includes(pokemon.baseSpecies)))
        continue;
      ...
      if (learnsetTest && this.modData("Learnsets", pokemonID) === void 0) {
      ...
      if (pokemon.battleOnly || ["Mega", "Mega-X", "Mega-Y", "Primal", "Ultra"].includes(pokemon.forme) || [
        ...
        "Terapagos",
        "Venomicon"
      ].includes(pokemon.baseSpecies) && pokemon.baseSpecies !== pokemon.name)
        continue;
      if (learnsetTest) {
      ...
      let moveLearn;
      let startGen;
      if (baseSeven.includes(pokemonID))
        startGen = 7;
      else if (pokemon.num > 898 || pokemon.num < -68 || baseNine.includes(pokemonID))
        startGen = 9;
      else if (pokemon.num > 807 || pokemon.num < -60 || baseEight.includes(pokemonID))
        startGen = 8;
      else
        startGen = 7;
      const levelString = new RegExp(startGen + "L[0-9]+");
      if (learnsetTest)
        console.log("Starting with Gen " + startGen);
      const stoneCheck = startGen === 7 && pokemon.prevo && !["Eevee", "Sunkern", "Charjabug", "Darumaka-Galar", "Capsakid"].includes(pokemon.prevo) && pokemon.evoItem && ["Fire Stone", "Water Stone", "Thunder Stone", "Leaf Stone", "Moon Stone", "Sun Stone", "Shiny Stone", "Dusk Stone", "Ice Stone"].includes(pokemon.evoItem);
      if (stoneCheck) {
        if (learnsetTest)
          console.log("This Pokemon evolves by Evolution Stone and needs its prevo's level-up moves");
        for (const moveID in this.modData("Learnsets", this.toID(pokemon.prevo)).learnset) {
          ...
          const esLevelString = new RegExp("9L[0-9]+");
          if (esLevelString.test(prevoMove[0])) {
            if (learnsetTest)
              console.log("Importing " + moveID);
            if (this.modData("Learnsets", pokemonID).learnset[moveID])
              this.modData("Learnsets", pokemonID).learnset[moveID].unshift("7L1");
            else
              this.modData("Learnsets", pokemonID).learnset[moveID] = ["7L1"];
          }
        }
        if (learnsetTest)
      ...
      for (let i = 0; i < renamedMoves.length; i++) {
        if (this.modData("Learnsets", pokemonID).learnset[renamedMoves[i]]) {
          if (learnsetTest)
            console.log("Renaming " + renamedMoves[i]);
      ...
      for (let i = 0; i < deletedMoves.length; i++) {
        if (this.modData("Learnsets", pokemonID).learnset[deletedMoves[i]]) {
          if (learnsetTest)
            console.log("Deleting " + deletedMoves[i]);
      ...
      for (let move of this.moves.all()) {
        ...
        if (!moveLearn) {
          if (!noUniversalTMs.includes(pokemonID)) {
            if (moveID === "endure" && pokemon.gen > 4) {
              if (learnsetTest)
                console.log("Adding universal TM Endure");
              ...
              if (learnsetTest)
                console.log("Adding universal TM " + move.name);
              this.modData("Learnsets", pokemonID).learnset[moveID] = ["9M"];
            } else if (moveID === "attract" && pokemon.gen === 9 && pokemon.gender !== "N") {
              if (learnsetTest)
                console.log("Adding universal TM Attract");
              this.modData("Learnsets", pokemonID).learnset[moveID] = ["9M"];
            } else if (moveID === "terablast" && pokemon.gen < 9) {
              if (learnsetTest)
                console.log("Adding universal Tutor Tera Blast");
        ...
        if (["grasspledge", "firepledge", "waterpledge", "frenzyplant", "blastburn", "hydrocannon", "dracometeor", "steelbeam", "meteorbeam", "terablast"].includes(moveID) && (moveLearn.includes("7T") || moveLearn.includes("8T") || moveLearn.includes("9M") || moveLearn.includes("7M") || moveLearn.includes("8M"))) {
          if (learnsetTest)
        ...
        if (!moveMeans.length) {
          ...
          if (startGen > 7 && moveLearn.includes("7E")) {
            if (learnsetTest)
              console.log("This move was learned by egg before it got removed");
            moveMeans.push("9E");
          }
        }
        if (learnsetTest)
          console.log("Compiled: " + moveMeans);
        if (moveID === "lashout" && moveMeans == "9M") {
          if (learnsetTest)
    ...
    this.modData("Learnsets", "deoxys").learnset.refresh = ["9D"];
    this.modData("Learnsets", "deoxys").learnset.meteormash = ["9L73"];
    this.modData("Learnsets", "deoxys").learnset.workup = ["9D"];
    this.modData("Learnsets", "deoxys").learnset.zapcannon = ["9L55"];
    this.modData("Learnsets", "deoxys").learnset.metalburst = ["9D"];
    this.modData("Learnsets", "deoxys").learnset.barrier = ["9L55"];
    this.modData("Learnsets", "deoxys").learnset.feint = ["9D"];
    this.modData("Learnsets", "deoxys").learnset.doubleteam = ["9L37"];
    this.modData("Learnsets", "deoxys").learnset.flash = ["9M"];
    ...
    this.modData("Learnsets", "basculinbluestriped").learnset = import_lib.Utils.deepClone(this.modData("Learnsets", "basculin").learnset);
    ...
    this.modData("Learnsets", "basculinwhitestriped").learnset = import_lib.Utils.deepClone(this.modData("Learnsets", "basculin").learnset);
    ...
    this.modData("Learnsets", "meloettapirouette").learnset = import_lib.Utils.deepClone(this.modData("Learnsets", "meloetta").learnset);
    ...
    this.modData("Learnsets", "squawkabillyblue").learnset = import_lib.Utils.deepClone(this.modData("Learnsets", "squawkabilly").learnset);
    this.modData("Learnsets", "squawkabillyyellow").learnset = import_lib.Utils.deepClone(this.modData("Learnsets", "squawkabilly").learnset);
    this.modData("Learnsets", "squawkabillywhite").learnset = import_lib.Utils.deepClone(this.modData("Learnsets", "squawkabilly").learnset);
    ...
    this.modData("Learnsets", "ogerponwellspring").learnset = import_lib.Utils.deepClone(this.modData("Learnsets", "ogerpon").learnset);
    ...
    this.modData("Learnsets", "ogerponhearthflame").learnset = import_lib.Utils.deepClone(this.modData("Learnsets", "ogerpon").learnset);
    ...
    this.modData("Learnsets", "ogerponcornerstone").learnset = import_lib.Utils.deepClone(this.modData("Learnsets", "ogerpon").learnset);
    ...
    this.modData("Learnsets", "brattler").learnset.powertrip = ["9L52"];
