...
const Conditions = {
  ...
  frz: {
    ...
    onBeforeMove(pokemon, target, move) {
      if (move.flags["defrost"] || pokemon.volatiles["nointerrupt"]?.ignore.includes("frz"))
        return;
  ...
  partiallytrapped: {
    ...
    onStart(pokemon, source) {
      if (pokemon.volatiles["strongpartialtrap"])
        return false;
  ...
  evade: {
    ...
    name: "evade",
    onStart(pokemon) {
      if (pokemon.volatiles["odorsleuth"])
        return false;
    },
  ...
  evadestall: {
    ...
    onEvadeStallMove(pokemon) {
      ...
      if (!success)
        delete pokemon.volatiles["evadestall"];
      return success;
    },
    onRestart() {
  ...
  nointerrupt: {
    ...
    onUpdate(pokemon) {
      ...
      for (const volatile in pokemon.volatiles) {
        ...
        if (["attract", "choicelock", "confusion", "disable", "encore", "flinch", "taunt", "throatchop"].includes(volatile) && !this.effectState.currentVolatiles.includes(volatile.id) && !this.effectState.ignore.includes(volatile)) {
  ...
  blocked: {
    ...
    onSourceHit(target, source, move) {
      if (move.selfSwitch && target !== source && !source.hasItem("shedshell") && !source.hasAbility("runaway"))
  ...
  arenatrapped: {
    ...
    onTrapPokemon(pokemon) {
      if (pokemon.isGrounded() && !pokemon.hasAbility("arenatrap"))
        pokemon.tryTrap();
    }
  },
  singletrap: {
  ...
  par: {
    ...
    onBeforeMove(pokemon) {
      if (pokemon.volatiles["nointerrupt"]?.ignore.includes("par"))
        return;
  ...
  slp: {
    ...
    onBeforeMove(pokemon, target, move) {
      ...
      if (pokemon.volatiles["nointerrupt"]?.ignore.includes("slp"))
        return;
  ...
  confusion: {
    ...
    onBeforeMove(pokemon) {
      ...
      if (pokemon.volatiles["nointerrupt"]?.ignore.includes("confusion"))
        return;
  ...
  flinch: {
    ...
    onBeforeMove(pokemon) {
      if (pokemon.volatiles["nointerrupt"]?.ignore.includes("flinch"))
        return;
  ...
  choicelock: {
    ...
    onBeforeMove(pokemon, target, move) {
      ...
      if (!pokemon.ignoringItem() && move.id !== this.effectState.move && move.id !== "struggle" && !pokemon.volatiles["nointerrupt"]?.ignore.includes("choicelock")) {
