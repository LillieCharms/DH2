...
const Moves = {
  ...
  indomitablespirit: {
    ...
    basePowerCallback(pokemon, target, move) {
      if (pokemon.moveLastTurnResult === false)
        return move.basePower * 2;
      if (pokemon.volatiles["indomitablespirit"].boost === "lastMoveResisted")
        return move.basePower * 2;
      return move.basePower;
    },
    onPrepareHit: function(target, source) {
  ...
  contrariety: {
    ...
    onHitField() {
      ...
      for (const pokemon of this.getAllActive()) {
        if (pokemon.ability === "truant" || pokemon.ability === "contrary" || pokemon.getAbility().flags["cantsuppress"])
          continue;
        const oldAbility = pokemon.setAbility("contrary");
        if (oldAbility)
          this.add("-ability", pokemon, "Contrary", "[from] move: Contrary");
        success = true;
      }
      return success;
    },
  ...
  ragingfury: {
    ...
    basePowerCallback(pokemon, target, move) {
      ...
      if (pokemon.status !== "slp")
        pokemon.addVolatile("ragingfury");
      this.debug("Rollout bp: " + bp);
      return bp;
    },
    condition: {
  ...
  conversion: {
    ...
    onHit(target) {
      ...
      if (target.hasType(type) || !target.setType(type) || target.hasItem("identitycard"))
        return false;
      this.add("-start", target, "typechange", type);
    }
  },
  conversion2: {
    ...
    onHit(target, source) {
      ...
      if (source.hasItem("identitycard"))
        return false;
  ...
  magicpowder: {
    inherit: true,
    onHit(target) {
      if (target.getTypes().join() === "Psychic" || !target.setType("Psychic") || target.hasItem("identitycard"))
        return false;
