...
const Abilities = {
  ...
  cursedbody: {
    onSourceModifyDamage(damage, source, target, move) {
      if (this.effectState.cursed)
        return;
      return this.chainModify(0.75);
    },
    onDamagingHit(damage, target, source, move) {
      if (this.effectState.cursed || source.volatiles["disable"])
        return;
  ...
  magician: {
    ...
    onStart(pokemon) {
      ...
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName("Merasmus")}|${this.sample(switchin)}`);
    },
    onResidual(pokemon) {
      if (pokemon.volatiles["substitute"]) {
        ...
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName("Merasmus")}|${this.sample(sub)}`);
    ...
    onSwitchOut(pokemon) {
      ...
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName("Merasmus")}|${this.sample(switchout)}`);
    },
    onFaint(pokemon) {
      const faint = [
        "Ach, no!",
      ...
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName("Merasmus")}|${this.sample(faint)}`);
    }
  },
  mutualexclusion: {
    onStart(target) {
  ...
  powerofalchemy: {
    ...
    onStart(pokemon) {
      if (!pokemon.isStarted || !this.effectState.switchingIn)
        return;
      ...
      const possibleTargets = pokemon.foes().filter((foeActive) => foeActive && !foeActive.getAbility().isPermanent && !additionalBannedAbilities.includes(foeActive.ability) && foeActive.isAdjacent(pokemon));
