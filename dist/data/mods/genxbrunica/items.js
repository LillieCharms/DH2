...
const Items = {
  ...
  interactivelens: {
    ...
    onSourceAccuracy(accuracy, target, source, move) {
      return move && move.category !== "Status" && !move.ohko || accuracy;
    },
  ...
  ashball: {
    ...
    onPrepareHit(target, source, move) {
      if (!move || move.id !== "fling")
        return;
      (move.secondaries || (move.secondaries = [])).push({ chance: 100, boosts: { accuracy: -1 } });
    },
  ...
  slushisloshiscale: {
    ...
    ignoreKlutz: true,
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Slushisloshi" || pokemon.transformed || pokemon.level < 20)
        return;
