...
const Moves = {
  ...
  cruelstrike: {
    ...
    onPrepareHit: function(target, source2, move) {
      ...
      this.add("-anim", source2, "Darkest Lariat", target);
    },
    onTry(source2) {
  ...
  gemdevour: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Bug Bite", target);
    },
  ...
  bullybash: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Dragon Rush", target);
    },
    pp: 5,
    priority: 0,
  ...
  psybolt: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Psycho Boost", target);
    },
    pp: 10,
    priority: 0,
  ...
  snowbank: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Ice Spinner", target);
    },
  ...
  smokebomb: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Smoke Screen", target);
    },
    secondaries: [
      {
        chance: 10,
  ...
  clarentcleave: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Ceaseless Edge", target);
    },
    onEffectiveness(typeMod, target, type) {
      if (type === "Fairy")
        return 1;
  ...
  sleepdrain: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Dream Eater", target);
    },
    onModifyMove(move, pokemon, target) {
  ...
  healingnature: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Seed Flare", target);
    },
    onEffectiveness(typeMod, target, type) {
      if (type === "Poison")
        return 1;
  ...
  magneticupdraft: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Wildbolt Storm", target);
    },
  ...
  leapingonrush: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Headlong Rush", target);
    },
  ...
  cupricdeluge: {
    ...
    onAfterSubDamage(damage, target, source2, move) {
      if (!move.hasSheerForce && source2.hp) {
        for (const side of source2.side.foeSidesWithConditions()) {
          side.addSideCondition("gmaxsteelsurge");
        }
  ...
  divebomb: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Wave Crash", target);
    },
    secondary: {
      chance: 100,
      onHit(target) {
  ...
  muddevourment: {
    ...
    onAfterHit(target, source2) {
      if (source2.hp && !this.field.isTerrain(["poisonterrain", "guardianofnature"])) {
  ...
  cloneexpress: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Spectral Thief", target);
    },
    secondary: {
      chance: 100,
      onHit(target, source2, move) {
        if (target.status) {
          target.addVolatile("confusion", source2, move);
        }
      }
    },
    target: "normal",
  ...
  oliverampage: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Outrage", target);
    },
    secondary: null,
    target: "normal",
    type: "Dragon"
  ...
  asurabarrage: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Hyper Drill", target);
    },
    self: {
  ...
  thunderarmor: {
    ...
    onHitSide(side, source2) {
      source2.addVolatile("stall");
    },
    condition: {
      duration: 1,
      onSideStart(target, source2) {
        this.add("-singleturn", source2, "Thunder Armor");
        source2.addVolatile("thunderarmorboost");
      },
      onTryHitPriority: 4,
      onTryHit(target, source2, move) {
        ...
        if (lockedmove && source2.volatiles["lockedmove"].duration === 2) {
  ...
  mysticburst: {
    ...
    onAfterHit(target, source2, move) {
      if (!this.field.pseudoWeather.trickroom && !move.hasSheerForce && source2.hp) {
  ...
  maidenspeak: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      if (!source2.volatiles["maidenspeak"]) {
  ...
  floralhealing: {
    ...
    onHit(target, source2) {
      ...
      if (this.field.isTerrain(["grassyterrain", "guardianofnature"])) {
        ...
        if (this.field.isTerrain("guardianofnature"))
          success = target.cureStatus() || success;
      } else {
  ...
  minimize: {
    ...
    condition: {
      ...
      onAccuracy(accuracy, target, source2, move) {
        const boostedMoves = [
          "stomp",
          "steamroller",
          "bodyslam",
  ...
  mindmelt: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Confusion", target);
    },
    secondary: {
      chance: 30,
      volatileStatus: "confusion"
  ...
  poisonousflight: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Mortal Spin", target);
    },
    selfSwitch: true,
    secondary: null,
    target: "normal",
  ...
  sleuth: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Foresight", target);
    },
  ...
  frostfeint: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Aurora Beam", target);
    },
    selfSwitch: true,
    secondary: null,
    target: "normal",
  ...
  magicspin: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Magical Torque", target);
    },
    secondary: {
      chance: 10,
      volatileStatus: "confusion"
  ...
  draconicfang: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Hyper Fang", target);
    },
    secondary: {
      chance: 30,
      status: "brn"
  ...
  poisonterrain: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Grassy Terrain", target);
    },
    ...
    condition: {
      ...
      onFieldStart(field, source2, effect) {
        if (effect?.effectType === "Ability") {
          this.add("-fieldstart", "move: Poison Terrain", "[from] ability: " + effect.name, "[of] " + source2);
        } else {
      ...
      onResidual(pokemon) {
        ...
        } else if (!pokemon.hasType(["Bug", "Poison", "Steel"]))
          this.damage(pokemon.baseMaxhp / 16, pokemon, pokemon);
      },
      onSetStatus(status, target, source2, effect) {
        if (status.id === "psn" && target.isGrounded() && !target.isSemiInvulnerable() && effect?.effectType === "Move") {
  ...
  rototiller: {
    ...
    onHitField(target, source2) {
      ...
      for (const pokemon of this.getAllActive()) {
        ...
        } else if (pokemon !== source2 && !pokemon.volatiles["substitute"]) {
          nonGrass.push(pokemon);
        }
      }
      const isNatureField = this.field.isTerrain("guardianofnature");
      if (!targets.length && !anyAirborne && (!nonGrass.length || !isNatureField))
        return false;
      let boost = 2;
      if (isNatureField) {
      ...
      for (const pokemon of targets) {
        this.boost({ atk: boost, spa: boost }, pokemon, source2);
      }
    }
  },
  ...
  toxicshock: {
    ...
    onModifyCritRatio(critRatio, source2, target) {
  ...
  airdive: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Beak Blast", target);
    },
    willCrit: true,
    secondary: null,
    target: "normal",
  ...
  pepperrush: {
    ...
    onAfterHit(target, source2) {
      if (source2.hp) {
        source2.trySetStatus("brn", source2);
      }
    },
    secondary: {
      chance: 10,
  ...
  inkburst: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      switch (source2.species.name) {
        case "Crayoct":
          this.add("-anim", source2, "Spicy Extract", target);
          break;
        case "Crayoct-Blue":
        ...
        case "Crayoct-Pink":
          ...
          break;
        case "Crayoct-Brown":
          this.add("-anim", source2, "Mud Bomb", target);
          break;
        default:
  ...
  voraciousfang: {
    ...
    onModifyMove(move, source2, target) {
      if (!["tox", "psn"].includes(target.status))
        return;
  ...
  royalpunt: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Mega Kick", target);
    },
    secondary: {
      chance: 100,
      boosts: {
  ...
  pitfall: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Dig", target);
    },
    ...
    condition: {
      ...
      onHit(target, source2, move) {
        if (this.checkMoveMakesContact(move, source2, target)) {
          source2.addVolatile("trapped", target, this.dex.getActiveMove("Pitfall"), "trapper");
        }
      }
    },
    onAfterMove(pokemon) {
  ...
  telekinesis: {
    ...
    onTry(source2, target, move) {
      if (this.field.getPseudoWeather("Gravity")) {
        this.attrLastMove("[still]");
        this.add("cant", source2, "move: Gravity", move);
        return null;
      } else if (["Diglett", "Dugtrio", "Palossand", "Sandygast"].includes(target.baseSpecies.baseSpecies) || target.baseSpecies.name === "Gengar-Mega") {
  ...
  haze: {
    ...
    onHitField() {
      ...
      for (const pokemon of this.getAllActive()) {
        if (!pokemon.hasAbility("rockbottom"))
          pokemon.clearBoosts();
      }
    }
  },
  ...
  smackdown: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon) {
        let applies = !(!(pokemon.hasType("Flying") || pokemon.hasAbility(["levitate", "soaringspirit"]) || this.getAllActive().some((target) => target.hasAbility("treetopper"))) || pokemon.hasItem("ironball") || pokemon.volatiles["ingrain"] || this.field.getPseudoWeather("gravity"));
  ...
  dive: {
    ...
    onTryMove(attacker, defender, move) {
      ...
      if (attacker.hasAbility(["gulpmissile", "gulpcannon"]) && ["Cramorant", "Cramorant-Desvega", "Toxirant"].includes(attacker.species.name) && !attacker.transformed) {
  ...
  knockoff: {
    ...
    onAfterHit(target, source2) {
      if (source2.hp) {
        ...
        if (item) {
          this.add("-enditem", target, item.name, "[from] move: Knock Off", "[of] " + source2);
          for (const pokemon of this.getAllActive()) {
            if (pokemon.hasAbility("ravenous"))
              this.heal(pokemon.maxhp / 6, pokemon, pokemon, this.dex.abilities.get("ravenous"));
          }
        }
      }
    }
  ...
  */
  sonicpulse: {
    ...
    onPrepareHit: function(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Supersonic", target);
    },
    condition: {
      duration: 0,
  ...
  flareup: {
    ...
    onModifyMove(move, source2, target) {
      if (source2.getStat("atk", false, true) < target.getStat("atk", false, true)) {
  ...
  purification: {
    ...
    onHit(target, source2, move) {
      const boosts = {};
      let i;
      for (i in source2.boosts) {
        if (source2.boosts[i] < 0) {
          boosts[i] = 0;
        }
      }
      source2.setBoost(boosts);
      this.battle.add("-clearnegativeboost", source2);
    },
    target: "normal",
    type: "Rock",
    contestType: "Beautiful"
  ...
  guardianwind: {
    ...
    condition: {
      ...
      onSetStatus(status, target, source2, effect) {
        if (!effect || !source2 || effect.id === "yawn" || effect.effectType === "Move" && effect.infiltrates && target.side !== source2.side)
          return;
        if (target !== source2) {
  ...
  psychicsurf: {
    ...
    secondary: {
      ...
      self: {
        onHit(target, source2, move) {
          return this.field.isTerrain("psychicterrain") && source2.isGrounded() && !!this.boost({ spe: 1 }, source2);
        }
      }
    },
    target: "allAdjacent",
  ...
  jawsoflife: {
    ...
    onHit(target, source2) {
      if (source2.side === target.side && !this.heal(Math.floor(target.baseMaxhp * 0.5))) {
        this.add("-immune", target);
      }
    },
    secondary: {
