...
const Abilities = {
  ...
  guardianofnature: {
    ...
    onTerrainChange(pokemon) {
      ...
      if (!strongTerrains.includes(terrain) && (this.field.terrainState.source !== pokemon && terrain || !(pokemon.beingCalledBack || pokemon.switchFlag)))
        this.field.setTerrain("guardianofnature");
    },
    onEnd(pokemon) {
      if (this.field.terrainState.source !== pokemon)
        return;
    ...
    condition: {
      ...
      onTryHit(target, source, effect) {
        if (effect && effect.id.endsWith("terrain") && this.dex.moves.get(effect.id))
          return false;
      },
  ...
  cloninggenes: {
    ...
    condition: {
      onStart(pokemon) {
        ...
        if (!pokemon.volatiles["substitute"] && pokemon.hp > threshold && pokemon.maxhp > 1 && pokemon.hasAbility("cloninggenes")) {
  ...
  prismshell: {
    onPrepareHit(source, target, move) {
      if (move.hasBounced || move.category !== "Status" || move.flags["futuremove"] || move.sourceEffect === "snatch")
        return;
  ...
  buzzing: {
    ...
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Moskitoski" || pokemon.level < 20 || pokemon.transformed)
        return;
      ...
      } else if (pokemon.species.id === "moskitoskiswarm") {
  ...
  dustdevil: {
    name: "Dust Devil",
    shortDesc: "The Pok\xE9mon's Attack is boosted by 50% if Sandstorm is active.",
  ...
  powerlock: {
    ...
    onAnyAfterBoost(boost, target, source, effect) {
      if (target.hasAbility("powerlock"))
        return;
  ...
  gulpcannon: {
    ...
    onSourceTryPrimaryHit(target, source, effect) {
      if (effect && effect.id === "surf" && source.hasAbility("gulpcannon") && ["Cramorant-Desvega", "Toxirant"].includes(source.species.name) && !source.transformed) {
  ...
  tacticalmonarch: {
    onStart(pokemon) {
      if (pokemon.m.monarch || pokemon.volatiles["tacticalmonarch"])
        return;
      for (const target of pokemon.foes()) {
  ...
  icecurse: {
    onStart(pokemon) {
      let activated = false;
      for (const target of this.getAllActive()) {
        if (target.hasType("Ice") || !target.isAdjacent(pokemon) || target.species.isMega && target.item?.endsWith("mask") || target.volatiles["substitute"] || !target.addType("Ice"))
          continue;
  ...
  schooling: {
    ...
    onStart(pokemon) {
      if (!["Slushisloshi", "Wishiwashi"].includes(pokemon.baseSpecies.baseSpecies) || pokemon.level < 20 || pokemon.transformed)
        return;
      if (pokemon.baseSpecies.baseSpecies === "Slushisloshi" && pokemon.hasItem("slushisloshiscale"))
        return;
      if (pokemon.hp > pokemon.maxhp / 4) {
  ...
  deepforest: {
    ...
    onAnyBasePower(basePower, source, target, move) {
      ...
      else if (move.auraBooster !== this.effectData.target)
        return;
  ...
  solarflare: {
    ...
    onWeatherChange(pokemon) {
      if (!pokemon.isActive || pokemon.baseSpecies.baseSpecies !== "Hyakada" || pokemon.transformed || !pokemon.hp)
        return;
  ...
  grassycloak: {
    ...
    onModifyAccuracy(accuracy) {
      if (typeof accuracy !== "number" || !this.field.isTerrain(["grassyterrain", "guardianofnature"]))
        return;
  ...
  mindprobe: {
    ...
    onStart(pokemon) {
      for (const target of pokemon.side.foe.active) {
        if (!target || target.fainted || !target.item)
          continue;
      ...
      for (const target of pokemon.side.foe.active) {
        ...
        for (const moveSlot of target.moveSlots) {
          ...
          if (move.ohko)
            bp = 150;
          else if (move.id === "counter" || move.id === "metalburst" || move.id === "mirrorcoat")
            bp = 120;
          else if (bp === 1 || !bp && move.category !== "Status")
            bp = 80;
          if (bp > warnBp) {
  ...
  */
  ...
  treetopper: {
    ...
    onAnyAccuracy(accuracy, target, source, move) {
      return move && !move.ohko && !target.isGrounded() && !["Diglett", "Dugtrio", "Palossand", "Sandygast"].includes(target.baseSpecies.baseSpecies) && target.baseSpecies.name !== "Gengar-Mega" || accuracy;
    },
    flags: {},
    name: "Tree-Topper"
    //rating: 0.1,
  ...
  phaseshift: {
    ...
    onPrepareHit(source, target, move) {
      ...
      } else if (move.type === "Ice" && source.getTypes().join() !== "Ice" && source.setType("Ice")) {
  ...
  bombadier: {
    shortDesc: "This Pok\xE9mon explosion, ball, and bomb moves have 1.3x power.",
    onBasePowerPriority: 21,
    onBasePower(basePower, attacker, defender, move) {
    ...
    onModifyMove(move) {
      if (move.secondaries) {
        ...
        for (const secondary of move.secondaries) {
          if (["direclaw", "spellcast", "triattack"].includes(move.id) || secondary.status && secondary.chance)
            secondary.chance *= 2;
        }
      }
      if (move.self?.status && move.self.chance)
        move.self.chance *= 2;
    },
    flags: {},
    name: "Bewitch"
  ...
  surfsup: {
    ...
    onBeforeMove(attacker, defender, move) {
      if (attacker.species.name !== "Tsunamey" || move.type !== "Water" || attacker.transformed)
        return;
      attacker.formeChange("Tsunamey-Surfing");
    },
    onWeatherChange(pokemon) {
      if (pokemon.isActive && pokemon.species.id === "tsunamey" && !pokemon.transformed && pokemon.hp && ["raindance", "primordialsea"].includes(pokemon.effectiveWeather()))
  ...
  battletide: {
    ...
    onAnyRedirectTarget(target, source, source2, move) {
      ...
      if (!this.validTarget(abilHolder, source, redirectTarget))
        return;
      move.smartTarget && (move.smartTarget = false);
  ...
  potionbrewer: {
    shortDesc: "(Bugged) Upon using a Psychic-type move, this Pok\xE9mon consumes its berry.",
    onSourceAfterMoveSecondary(target, source, move) {
      if (move.type === "Psychic" && source.getItem()?.isBerry)
        source.eatItem(true);
    },
    flags: {},
    name: "Potion Brewer"
