...
const Abilities = {
  ...
  everlastingwinter: {
    desc: "On switch-in, the weather becomes Hail. This weather remains in effect until this Ability is no longer active for any Pok\xE9mon, or the weather is changed by Delta Stream, Desolate Land or Primordial Sea. Super effective moves only inflict 3/4 damages on this Pokemon.",
  ...
  iceface: {
    onDamage(damage, target, source, effect) {
      if (effect && effect.effectType === "Move" && target.species.id === "eiscue" && !target.transformed) {
    ...
    onCriticalHit(target, type, move) {
      if (!target)
        return;
      if (target.species.id !== "eiscue" || target.transformed)
        return;
