...
const Abilities = {
  ...
  */
  ...
  tigerpit: {
    ...
    onModifyDamage(damage, source, target, move) {
      if (move && target.isGrounded() || !target.hasAbility("feelnopain") || !target.hasAbility("magneticwaves") || !target.hasAbility("stickyfloat") || !target.hasAbility("etativel") || !target.hasAbility("lighthearted") || !target.hasAbility("leviflame") || !target.hasAbility("levitability") || !target.hasAbility("feelsomepain") || !target.hasAbility("aerialbreak") || !target.hasAbility("floatguise") || !target.hasAbility("clearlyfloating") || !target.hasAbility("hoverboard") || !target.hasAbility("levimetal") || !target.hasAbility("levistatic") || !target.hasAbility("lovelessfloat") || target.hasAbility("testcram") || !target.hasType("Ghost")) {
        return this.chainModify(1.5);
      }
    },
    name: "Tiger Pit",
  ...
  */
  ...
  toxinrush: {
    onModifyMove(move) {
      ...
      if (move.secondaries) {
        this.debug("doubling secondary chance");
        for (const secondary of move.secondaries) {
          if (this.field.isWeather("hail") && secondary.chance && secondary.status === "psn")
            secondary.chance *= 2;
        }
      }
    },
  ...
  bombshell: {
    ...
    */
    ...
    onResidualOrder: 27,
    onResidual(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Minimie" || pokemon.transformed || !pokemon.hp)
        return;
  ...
  */
  ...
  magicarmour: {
    onBoost(boost, target, source, effect) {
      if (target === source || !boost || effect.id === "mirrorarmor" || effect.id === "magicarmour")
        return;
      let b;
      for (b in boost) {
        if (boost[b] < 0) {
  ...
  ultrahaircut: {
    ...
    onDamagingHit(damage, target, source, move) {
      if (!move || !move.flags["contact"] || target.volatiles["substitute"])
        return;
      let statName = "atk";
      let bestStat = 0;
      let s;
