...
const Moves = {
  ...
  smackdown: {
    ...
    condition: {
      ...
      onStart(pokemon) {
        let applies = false;
        if (pokemon.hasType("Flying") || pokemon.hasAbility("levitate") || pokemon.hasAbility("magneticwaves") || pokemon.hasAbility("leviflame") || pokemon.hasAbility("levitability") || pokemon.hasAbility("stickyfloat") || pokemon.hasAbility("etativel") || pokemon.hasAbility("lighthearted") || pokemon.hasAbility("clearlyfloating") || pokemon.hasAbility("floatguise") || pokemon.hasAbility("aerialbreak") || pokemon.hasAbility("levimetal") || pokemon.hasAbility("hoverboard") || pokemon.hasAbility("levistatic") || pokemon.hasAbility("lovelessfloat") || pokemon.hasAbility("ghoulaway") || pokemon.hasAbility("spiritascent") || pokemon.hasAbility("testcram") || pokemon.hasAbility("floatingreach"))
          applies = true;
  ...
  thousandarrows: {
    ...
    onModifyMove(move, source, target) {
      if (target.hasAbility("magneticwaves") || target.hasAbility("leviflame") || target.hasAbility("levitability") || target.hasAbility("stickyfloat") || target.hasAbility("etativel") || target.hasAbility("lighthearted") || target.hasAbility("clearlyfloating") || target.hasAbility("floatguise") || target.hasAbility("aerialbreak") || target.hasAbility("levimetal") || target.hasAbility("hoverboard") || target.hasAbility("levistatic") || target.hasAbility("lovelessfloat") || target.hasAbility("ghoulaway") || target.hasAbility("spiritascent") || target.hasAbility("testcram") || target.hasAbility("floatingreach")) {
  ...
  mistyterrain: {
    ...
    condition: {
      ...
      onSetStatus(status, target, source, effect) {
        if (!target.isGrounded() || target.isSemiInvulnerable() || source.hasAbility("mystic"))
          return;
  ...
  psychicterrain: {
    ...
    condition: {
      ...
      onTryHit(target, source, effect) {
        ...
        if (target.isSemiInvulnerable() || target.side === source.side || source.hasAbility("mystic"))
          return;
  ...
  hex: {
    ...
    basePowerCallback(pokemon, target, move) {
      if (target.status || target.hasAbility("comatose") || target.hasAbility("shadowydreams"))
  ...
  snore: {
    ...
    onTryHit(target, source) {
      if (source.status !== "slp" && !source.hasAbility("comatose") && !source.hasAbility("shadowydreams"))
        return false;
    },
    secondary: {
      chance: 30,
  ...
  wakeupslap: {
    ...
    basePower: 70,
    basePowerCallback(pokemon, target, move) {
      if (target.status === "slp" || target.hasAbility("comatose") || target.hasAbility("shadowydreams"))
        return move.basePower * 2;
      return move.basePower;
    },
    category: "Physical",
