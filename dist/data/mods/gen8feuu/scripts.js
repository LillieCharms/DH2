...
const Scripts = {
  ...
  pokemon: {
    ...
    isGrounded(negateImmunity = false) {
      ...
      if (!negateImmunity && this.hasType("Flying") && !("roost" in this.volatiles))
        return false;
      if ((this.hasAbility("levitate") || this.hasAbility("magneticwaves") || this.hasAbility("leviflame") || this.hasAbility("levitability") || this.hasAbility("stickyfloat") || this.hasAbility("etativel") || this.hasAbility("lighthearted") || this.hasAbility("clearlyfloating") || this.hasAbility("floatguise") || this.hasAbility("aerialbreak") || this.hasAbility("levimetal") || this.hasAbility("hoverboard") || this.hasAbility("levistatic") || this.hasAbility("ghoulaway") || this.hasAbility("spiritascent") || this.hasAbility("testcram") || this.hasAbility("floatingreach") || this.hasAbility("lovelessfloat")) && !this.battle.suppressingAttackEvents())
        return null;
    ...
    setStatus(status, source = null, sourceEffect = null, ignoreImmunities = false) {
      ...
      if (!ignoreImmunities && status.id && !(source?.hasAbility(["corrosion", "toxicplay", "deadlydeft"]) && ["tox", "psn"].includes(status.id))) {
  ...
  canUltraBurst(pokemon) {
    ...
    if (["Phancrozma-Dawn-Wings"].includes(pokemon.baseSpecies.name) && pokemon.getItem().id === "depletedultranecroziumz") {
