...
const Scripts = {
  ...
  pokemon: {
    ...
    isGrounded(negateImmunity = false) {
      ...
      if (!negateImmunity && this.hasType("Flying") && !("roost" in this.volatiles))
        return false;
      if ((this.hasAbility("levitate") || this.hasAbility("powerofalchemyweezing") || this.hasAbility("powerofalchemymismagius")) && !this.battle.suppressingAttackEvents())
        return null;
    ...
    ignoringAbility() {
      ...
      for (const pokemon of this.battle.getAllActive()) {
        if (pokemon.ability === "neutralizinggas" && !pokemon.volatiles["gastroacid"] && !pokemon.abilityData.ending || pokemon.ability === "powerofalchemyweezing" && !pokemon.volatiles["gastroacid"] && !pokemon.abilityData.ending) {
          neutralizinggas = true;
          powerofalchemyweezing = true;
          break;
        }
      }
      return !!(this.battle.gen >= 5 && !this.isActive || (this.volatiles["gastroacid"] || neutralizinggas && this.ability !== "neutralizinggas" || powerofalchemyweezing && this.ability !== "powerofalchemyweezing") && !this.getAbility().isPermanent);
    },
    setStatus(status, source = null, sourceEffect = null, ignoreImmunities = false) {
      ...
      if (!ignoreImmunities && status.id && !(source?.hasAbility(["corrosion", "powerofalchemymismagius", "powerofalchemyumbreon"]) && ["tox", "psn"].includes(status.id))) {
  ...
  */
  ...
  getAbility() {
    ...
    return item.exists ? item : this.battle.dex.abilities.get(this.ability);
  },
  hasItem(item) {
  ...
  setAbility(ability, source, isFromFormeChange) {
    if (this.battle.dex.items.get(this.ability).exists)
      return false;
  ...
  takeDual(source) {
    ...
    if (!source)
      source = this;
    const dual = this.getAbility();
    if (dual.effectType !== "Item")
      return false;
