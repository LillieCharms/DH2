...
const Abilities = {
  ...
  forecast: {
    ...
    onWeatherChange(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Cyclonimbus" || pokemon.transformed)
        return;
      let forme = null;
      switch (pokemon.effectiveWeather()) {
        case "sunnyday":
        case "desolateland":
          if (pokemon.species.id !== "cyclonimbussunny")
            forme = "Cyclonimbus-Sunny";
          break;
        case "raindance":
        case "primordialsea":
          if (pokemon.species.id !== "cyclonimbusrainy")
            forme = "Cyclonimbus-Rainy";
          break;
        case "hail":
        case "snow":
          if (pokemon.species.id !== "cyclonimbussnowy")
            forme = "Cyclonimbus-Snowy";
          break;
        default:
          if (pokemon.species.id !== "cyclonimbus")
            forme = "Cyclonimbus";
          break;
      }
  ...
  magician: {
    onStart(pokemon, move) {
      if (pokemon.hp === pokemon.maxhp) {
        ...
        if (!target || !pokemon.m.previousPartner)
          return;
        ...
        if (target.item || previousPartner.item || !yourItem && !myItem) {
          ...
          if (myItem)
            previousPartner.item = myItem.id;
          return;
        }
        if (myItem && !this.singleEvent("TakeItem", myItem, previousPartner.itemState, target, previousPartner, move, myItem) || yourItem && !this.singleEvent("TakeItem", yourItem, target.itemState, previousPartner, target, move, yourItem)) {
          if (yourItem)
            target.item = yourItem.id;
          if (myItem)
            previousPartner.item = myItem.id;
        ...
        if (!myItem.id && !yourItem.id) {
          ...
          this.add("-activate", previousPartner, "move: Trick", "[of] " + target);
        }
        if (myItem) {
          if (!myItem.id && !yourItem.id)
            return;
  ...
  stancechange: {
    ...
    onModifyMove(move, attacker, defender) {
      if (attacker.species.baseSpecies !== "Falinks" || attacker.transformed)
        return;
      if (move.category === "Status" && move.id !== "spikyshield")
        return;
