...
const Scripts = {
  ...
  pokemon: {
    ...
    isGrounded(negateImmunity = false) {
      ...
      if (!negateImmunity && this.hasType("Flying") && !(this.hasType("???") && "roost" in this.volatiles))
        return false;
      if ((this.hasAbility("levitate") || this.hasAbility("sunlitflight") || this.hasAbility("airdrive")) && !this.battle.suppressingAbility(this))
        return null;
  ...
  actions: {
    ...
    useMoveInner(moveOrMoveName, pokemon, target, sourceEffect, zMove, maxMove) {
      ...
      if (!move.negateSecondary && !(move.hasSheerForce && pokemon.hasAbility(["sheerforce", "sirocco", "strongarmor", "sheerbird"])) && !move.flags["futuremove"]) {
